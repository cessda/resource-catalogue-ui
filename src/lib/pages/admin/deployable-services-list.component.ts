...
export class DeployableServicesListComponent implements OnInit {
  ...
  deployableServicesForAudit: DeployableServiceBundle[] = [];
  selectedDeployableService: DeployableServiceBundle;
  facets: any;
  searchFacet = '';
  ...
  handleChange() {
    ...
    // console.log('map', map);
    this.router.navigate([`/provider/deployable-service/all`], {queryParams: map});
    // this.getServices();
  }
  ...
  getRandomResources(quantity: string) {
    ...
    this.resourceService.getRandomResources(quantity).subscribe(
      res => {
        this.deployableServicesForAudit = res['results'];
  ...
  showDeletionModal(dsBundle: DeployableServiceBundle) {
  ...
  showSuspensionModal(dsBundle: DeployableServiceBundle) {
    this.selectedDeployableService = dsBundle;
    if (this.selectedDeployableService) {
  ...
  showSendMailModal(dsBundle: DeployableServiceBundle) {
    this.selectedDeployableService = dsBundle;
    if (this.selectedDeployableService) {
  ...
  showMoveResourceModal(dsBundle: DeployableServiceBundle) {
  ...
  templateAction(id, active, status) {
    this.loadingMessage = '';
    UIkit.modal('#spinnerModal').show();
    this.deployableServiceService.verifyDeployableService(id, active, status).subscribe(
      res => {
        this.getProviders();
      },
      err => {
  ...
  moveResourceToProvider(deployableServiceId, providerId) {
    UIkit.modal('#spinnerModal').show();
  ...
  showAuditForm(view: string, dsBundle: DeployableServiceBundle) {
  ...
  editResourceInNewTab(providerId, deployableServiceId) {
    window.open(`/provider/${providerId}/deployable-service/update/${deployableServiceId}`, '_blank');
  }
